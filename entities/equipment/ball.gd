...
const VELOCITY_DAMAGE_THRESHOLD = 2.5
...
func _on_body_entered(body: Node3D):
	...
	var impact_force := last_velocity.distance_squared_to(linear_velocity)

	if impact_force < VELOCITY_DAMAGE_THRESHOLD * VELOCITY_DAMAGE_THRESHOLD:
		return
